...
+ [新增] 生图工作台新增电商套图模式，支持产品分析、镜头提示词规划、按镜头独立生成、视觉复核和原图交付。
+ [新增] 电商套图结果卡支持单张重新生成，重生成后清除旧复核结论并安全清理未再引用的旧原图。
+ [修复] 电商套图单张重新生成不再清除其他镜头的复核状态，并支持从结果卡单独复核单张生成图。
+ [修复] 电商套图复核按实际请求内容分批并拒绝单批超限请求，批量上传参考图失败时清理已上传文件。
+ [调整] 生图记录统一区分单图与电商套图两类，删除记录时只清理不再被我的素材或画布引用的原图。
+ [修复] 电商套图异步任务现在按任务隔离，提示词或产品信息变化时会使旧结果失效，关闭镜头后不再参与复核和交付。
+ [修复] 电商套图编辑与重新规划不再删除已有原图，并修复失效状态、分批复核、参考图压缩、配置检查和空草稿记录问题。
+ [修复] 电商套图结果卡操作按钮改为响应式网格布局，避免窄卡片中的长文案互相覆盖。
